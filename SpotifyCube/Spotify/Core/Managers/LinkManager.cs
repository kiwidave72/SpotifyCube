...
namespace Spotify.Core.Managers
{
    internal class LinkManager
    {
            ...
            TimeSpan? trackLinkOffset = null)
        {
            lock (_instanceLock)
            {
                ...
                if (!_instances.TryGetValue(handle, out instance))
                {
                    ...
                    {
                        type = Spotify.Core.Native.Spotify.sp_link_type(handle);
                    }
        ...
        {
            int bufferSize = Spotify.Core.Native.Spotify.STRINGBUFFER_SIZE;
            ...
            try
            {
                ...
                lock (Spotify.Core.Native.Spotify.Mutex)
                {
                    length = Spotify.Core.Native.Spotify.sp_link_as_string(linkHandle, builder, bufferSize);
                }
